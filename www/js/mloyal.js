...
var gbrd='7BF8E5D9-CAA5-4C6D-8F03-0383D48F3C5C';
...
var BrandNameShow='Purple Turtle';


var loginserver ="https://purpleturtleb2b.mloyalwoo.com/";
...
$(document).on('pageshow', '#homepage', function (event, ui) {
			...
			if(localStorage.getItem("usernamemloyal_share")=='helpdesk' || localStorage.getItem("usernamemloyal_share")=='mapp_login' || localStorage.getItem("usernamemloyal_share")=='PT278')
			{
...
function checkProfilepic()
{
	...
	var image_url2 = versionurl+'mliveshareshop/uploads/profile_'+gbrd+'.jpg';
    ...
    image.onload = function()
    {
    	...
    	$('#ppichome').attr("src",image.src);
    	$('#cardImagemenu').attr("src",image2.src);
        //document.getElementById("ppichome").src = image.src;
		//document.getElementById("cardImagemenu").src = image.src;
    }
    image.onerror = function()
    {
    	$('#ppichome').attr("src",'assets/images/shop_home.png');
    	$('#cardImagemenu').attr("src",'assets/images/user_pic.png');
...
function checkforundefined_prdtype(str)
{
    if(typeof str==undefined || str=='undefined'||str==null||str=='null')
    {
        str='simple';
...
function receivedEvent(id) {
 ...
 var push = PushNotification.init({ "android": {"senderID": "119804564475"}});
...
function addCategpry()
{
   ...
   var parentcat = $("#cat_main1").val();

   var store=gbrd;
   var picname=localStorage.getItem('catpic');
   ...
   else if(parentcat=='null' || parentcat==null || parentcat=='undefined' || parentcat=='')
   {
        ...
        var str = '';
			  ...
			  for (var i = 0; i < data.length; i++)
			  {
			        ...
			        if(typeof data[i].subcategory==undefined || typeof data[i].subcategory=="undefined" || data[i].subcategory==undefined || data[i].subcategory==null || data[i].subcategory=='' || data[i].subcategory=='null')
                    {
						...
						str+='<tr>';
						str+='<td style="background-color:#f1f1f1;font-weight:500;">'+checkforundefined(data[i].name)+' (Main)</td>';
						str+='<td style="background-color:#f1f1f1;">'+checkforundefined(data[i].gst)+ '</td>';
						str+='<td style="background-color:#f1f1f1;"><div class="manage_img"><img src="'+catimg+'" class="img-responsive" alt=""/><div></td>';
						str+='</tr>';
                    ...
                    else
                    {
						...
						var catimg2 = data[i].imagefolder;
						if(typeof catimg2 == undefined || typeof catimg2 == 'undefined' || catimg2 == null || catimg2 == 'null' || catimg2 == '')
						{
							     catimg2 = 'assets/images/no-img.jpg';   
						}

						str+='<tr>';
						str+='<td style="background-color:#f1f1f1;font-weight:500;">'+checkforundefined(data[i].name)+'(Main)</td>';
						str+='<td style="background-color:#f1f1f1;">'+checkforundefined(data[i].gst)+ '</td>';
						str+='<td style="background-color:#f1f1f1;"><div class="manage_img"><img src="'+catimg2+'" class="img-responsive" alt=""/><div></td>';
						str+='</tr>';
						...
						for(var k=0;k<data[i].subcategory.length;k++)
						{
							...
							str+='<tr class="catrow'+data[i].id+'">';
							str+='<td>'+checkforundefined(data[i].subcategory[k].name)+'</td>';
							str+='<td>'+checkforundefined(data[i].subcategory[k].gst)+ '</td>';
							str+='<td><div class="manage_img"><img src="'+catimg+'" class="img-responsive" alt=""/><div></td>';
							str+='</tr>';
		...
		success: function(data, textStatus, xhr) {
				...
				for(var i=0;i<data.length;i++)
				{
                    ...
                    if(typeof data[i].subcategory==undefined || typeof data[i].subcategory=="undefined" || data[i].subcategory==undefined || data[i].subcategory==null || data[i].subcategory=='' || data[i].subcategory=='null')
                    {
						...
						str +='</a>';
						str +='<div class="ui-grid-b social_share_list"><div class="ui-block-a"> <img src="assets/images/wapp.png" class="img-responsive" onclick="javascript:shareProductsCat(\''+data[i].id+'\',\''+data[i].name+'\',\''+catimg+'\');"></div><div class="ui-block-b"> <img src="assets/images/fb.png" class="img-responsive" onclick="javascript:shareProductsCatFb(\''+data[i].id+'\',\''+data[i].name+'\',\''+catimg+'\');"></div><div class="ui-block-c"> <img src="assets/images/instagram.png" class="img-responsive" onclick="javascript:shareProductsCatInsta(\''+data[i].id+'\',\''+data[i].name+'\',\''+catimg+'\');"></div></div>';
					    str +='</div>';
...
function shareProductsCatInsta(catcode,catname,iimage)
{
...
console.log(json.link);
 ...
 var des=json.link;
   ...
   try{
     ...
     (null,iimage, null,
      function() {document.getElementById('simages').src='assets/images/verify_icon.png';shareDescriptionInsta(''+des+'');}, 
      function(errormsg){alert(errormsg)});
...
function shareDescriptionInsta(desc)
{
	  try{
     window.plugins.socialsharing.shareViaInstagram
     (
    ...
    success: function(data, textStatus, xhr) {
        ...
        var str = '';
			  ...
			  for (var i = 0; i < data.length; i++)
			  {
				...
				str+='<tr>';
				str+='<td><span class="pr_nm" onclick="javascript:editProduct(\''+checkforundefined(data[i].id)+'\',\''+checkforundefined(data[i].itemcode)+'\',\''+checkforundefined(data[i].itemname)+'\',\''+checkforundefined(data[i].mrp)+'\',\''+checkforundefined(data[i].sale_price)+'\',\''+checkforundefined(data[i].catid)+'\',\''+description.trim().replace(/\"/g, "").replace(/[\n\r]/g, '\\n')+'\',\''+checkforundefined_prdtype(data[i].product_type)+'\',\''+checkforundefined(data[i].stock)+'\');">'+checkforundefined(data[i].itemname)+' ('+checkforundefined(data[i].description)+')</span></td>';
...
function editProduct(id,itemcode,itemname,mrp,sale_price,catid,description,product_type,stock)
{

   
   $('#prod_id2').val(id);
   $('#prod_itemcode2').val(itemcode);
   $('#prod_stock2').val(stock);
   $('#prod_type2').val(product_type);

   $('#prod_name2').val(itemname);
   $('#prod_desc2').val(description);
   $('#prod_price2').val(mrp);
   $('#prod_saleprice2').val(sale_price);
   $("#prod_cat2").val(catid);


   $.mobile.changePage('#edit_product');
...
function updateProduct()
{
   ...
   var id = $('#prod_id2').val();
   var itemcode = $('#prod_itemcode2').val();
   var stock = $('#prod_stock2').val();
   var prdtype = $('#prod_type2').val();

   var prdname = $('#prod_name2').val();
   var prddesc = $('#prod_desc2').val();
   var prdprice = $('#prod_price2').val();
   var prdsaleprice = $('#prod_saleprice2').val();
   var prdcat = $("#prod_cat2").val();


   if(id=='' || prdname=='' || prdprice=='')
   {
		toast('Please enter mandatory fields!');
   }
   else
...
function authenticate(){
 ...
 if(new_app=='true')
 {
					...
					document.getElementById('user_id_home').innerHTML = BrandNameShow;//data.data[0].phone;
					...
					document.getElementById('profilenm').innerHTML =BrandNameShow;// data.data[0].first_name;
  ...
  $.ajax({
   ...
   console.log('Data2:'+JSON.stringify(data));
        var listItems= "";
			...
			//listItems+= "<option value='0'>Main</option>";
        for (var i = 0; i < data.length; i++)
			{
